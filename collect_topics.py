...
Sources:
  ...
  - Google Trends (free, no key required, via pytrends)
...
HEADERS = {"User-Agent": "quicktechsolve-topic-bot/1.0"}
...
def collect_reddit():
    ...
    for sub in SUBREDDITS:
        try:
            ...
            print(f"[reddit] skipped r/{sub}: {e}", file=sys.stderr)
    return items
...
def collect_youtube():
    ...
    if not api_key:
        print("[youtube] skipped: YOUTUBE_API_KEY not set", file=sys.stderr)
        return []
    items = []
    try:
        url = (
...
def collect_twitter():
    ...
    if not token:
        print("[twitter] skipped: TWITTER_BEARER_TOKEN not set (paid API tier required)", file=sys.stderr)
        return []
    items = []
    try:
